...
class InstanceSegmentationRequestTest extends TestCase
{
    ...
    // Expanded test for Cropping, FV-Generation
    public function testFVGeneration()
    {
        Queue::fake();
        FileCache::fake();
        config([
        ...
        $inferenceInputJsonPath = "{$tmpDir}/input-inference.json";
        // Crop test
        $croppingInputJsonPath = "{$tmpDir}/input-cropping.json";
        $croppingOutputJsonPath = "{$tmpDir}/output-cropping.json";
        // FV test
        $dinofvInputJsonPath = "{$tmpDir}/input-dinofv.json";
        $dinofvOutputJsonPath = "{$tmpDir}/output-dinofv.json";
        ...
        // TODO: Match new Json checks to actual Jsons being generated
        // Cropping
        $expectCroppingJson = [
            'tmp_dir' => $tmpDir,
            'max_workers' => 2,
            'output_path' => "{$tmpDir}/output-cropping.json",
            'resize_dimension' => 512,
        ];

        // DinoFV
        $expectDinoFVJson = [
            'cropped_images_path' => "{$tmpDir}/cropped_images",
            'resize_dimension' => 512,
            'backbone' => "dino_vits16",
            'tmp_dir' => $tmpDir,
            'max_workers' => 2,
            'output_path' => "{$tmpDir}/output-dinofv.json",
        ];

        try {
            $request = new IsJobStub2($job);
            $request->handle();
            ...
            // Cropping
            $this->assertTrue(File::exists($croppingInputJsonPath));
            $inputJson = json_decode(File::get($croppingInputJsonPath), true);
            $this->assertArrayHasKey('images', $inputJson);
            $this->assertArrayHasKey('annotations', $inputJson);
            unset($inputJson['images']);
            unset($inputJson['annotations']);
            $this->assertEquals($expectCroppingJson, $inputJson);
            $this->assertStringContainsString("CropImages.py {$croppingInputJsonPath}", $request->commands[3]);

            // DinoFV
            $this->assertTrue(File::exists($dinofvInputJsonPath));
            $inputJson = json_decode(File::get($dinofvInputJsonPath), true);
            $this->assertArrayHasKey('cropped_images', $inputJson);
            unset($inputJson['cropped_images']);
            $this->assertEquals($expectDinoFVJson, $inputJson);
            $this->assertStringContainsString("DinoFVGenerator.py {$dinofvInputJsonPath}", $request->commands[4]);
...
// Used for Cropping and DinoFV test
class IsJobStub2 extends InstanceSegmentationRequest
{
    public $commands = [];
    public $cleanup = false;
    ...
    protected function python($command, $log = 'log.txt')
    {
        ...
        } elseif (Str::contains($command, 'CropImages')) {
            File::put("{$this->tmpDir}/output-cropping.json", '{}');
        } elseif (Str::contains($command, 'DinoFVGenerator')) {
            File::put("{$this->tmpDir}/output-dinofv.json", '{}');
        }
    }
